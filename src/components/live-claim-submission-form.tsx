...
export function LiveClaimSubmissionForm() {
  ...
  const [description, setDescription] = useState("Stage 6 live payout acceptance claim");
    ...
    async function load() {
      try {
        ...
        if (!response.ok || !result.workspace) {
          throw new Error(result.error ?? "Live treasury workspace could not be loaded.");
        }
        if (cancelled) return;
        setWorkspace(result.workspace);
        setCategoryReference(result.workspace.categories[0]?.externalReference ?? "");
      } catch (caught) {
        if (!cancelled) {
          setError(caught instanceof Error ? caught.message : "Live workspace could not be loaded.");
        }
      } finally {
  ...
  const selectedCategory = useMemo(
    () => workspace?.categories.find((category) => category.externalReference === categoryReference),
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !result.claim) {
        throw new Error(result.error ?? "The live claim could not be submitted.");
      }
      router.push(`/dashboard/claims/review?claim=${result.claim.id}`);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "The live claim could not be submitted.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (loading) {
    return <p className="rounded-2xl border border-[var(--line)] bg-white p-5 text-sm">Loading persisted live treasury…</p>;
  }

  return (
    <form className="rounded-3xl border border-[var(--line)] bg-white p-5 sm:p-8" onSubmit={submit}>
      <div className="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
        ...
        <p className="mt-1 break-all font-mono text-xs">{workspace?.treasuryObjectId}</p>
      </div>

      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="Member name">
          <input className={inputClass} value={submitterName} onChange={(e) => setSubmitterName(e.target.value)} required />
        </Field>
        <Field label="Merchant">
          <input className={inputClass} value={merchant} onChange={(e) => setMerchant(e.target.value)} required />
        </Field>
        ...
        <Field label="Budget category">
          <select className={inputClass} value={categoryReference} onChange={(e) => setCategoryReference(e.target.value)} required>
            <option value="">Choose a category</option>
            {workspace?.categories.map((category) => (
              <option key={category.externalReference} value={category.externalReference}>
                {category.name} · {formatUsdcMinor(category.allocatedMinor - category.spentMinor)} remaining
              </option>
            ))}
          </select>
        </Field>
        <Field label="Requested amount">
          <input className={inputClass} inputMode="decimal" value={requestedAmount} onChange={(e) => setRequestedAmount(e.target.value)} required />
        </Field>
        <Field label="Receipt reference">
          <input className={inputClass} value={receiptReference} onChange={(e) => setReceiptReference(e.target.value)} />
        </Field>
        <Field label="Recipient Sui address">
          <input className={`${inputClass} font-mono text-xs`} value={recipient} onChange={(e) => setRecipient(e.target.value)} required />
        </Field>
        <div className="sm:col-span-2">
          <Field label="Receipt image">
            <input
              ...
              type="file"
              onChange={(e) => setReceipt(e.target.files?.item(0) ?? null)}
              required
            />
          </Field>
        </div>
      ...
      {error && <p className="mt-5 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{error}</p>}
      ...
      <p className="mt-3 text-xs text-[var(--muted)]">
        This form uses persisted Supabase treasury/category values. Payment still requires a separate human approval and wallet signature.
      </p>
    </form>
  );
}
